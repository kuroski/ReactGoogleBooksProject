...
describe('BookShelf', () => {
  ...
  it('should render a Book when a list of books is passed', () => {
    ...
    const wrapper = mount(<BookShelf onFavorite={() => ''} isOnFavorite={() => ''} books={booksJson.items} term="Harry Potter" />)
    const onFavorite = wrapper.prop('onFavorite')
    const isOnFavorite = wrapper.prop('isOnFavorite')
    expect(wrapper.containsAllMatchingElements([
      <Book key={0} bookId={firstBook.id} index={0} title={firstBook.volumeInfo.title} onFavorite={onFavorite} isOnFavorite={isOnFavorite} term="Harry Potter" />
    ])).toEqual(true)
  })

  it('should render zero books', () => {
    const wrapper = shallow(<BookShelf onFavorite={() => ''} isOnFavorite={() => ''} books={[]} term="Harry Potter" />)
    expect(wrapper.find(Book)).toHaveLength(0)
    expect(wrapper.find('.c-book__favorite')).toHaveLength(0)
  })

  it('should render some books', () => {
    const wrapper = shallow(<BookShelf onFavorite={() => ''} isOnFavorite={() => ''} books={booksJson.items} term="Harry Potter" />)
